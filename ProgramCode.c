...
float savedFigures[300][10];
POINT polygonsCoords[150][400];
int polygonsCount = 0;
...
int WinInit() {
    textFont1 = CreateFont(30, 0, 0, 0, FW_MEDIUM, 0, 0, 0, 1, 0, 0, 0, 0, "Calibri");
...
void ShowCoordsSystem(HDC dc) {
    ...
    for (int i = -100; i < 100; i++){
        sprintf(sym1, "%.0f", i*systemParam);
        sprintf(sym2, "%.0f", -i*systemParam);

        MyText(dc, windowSize[0] / 2 + i * systemSize, windowSize[1] / 2, sym1);
        MyLine(dc, i * systemSize, -10000, i * systemSize, 10000);

        MyText(dc, windowSize[0] / 2, windowSize[1] / 2 - i * systemSize, sym2);
...
int PrintEndCode() {
    ...
    for (int i = 0; i < savedCount; i++){
        ...
        float sizeMaster = savedFigures[i][7];

        if (!savedFigures[i][6]) {
            printf("    SelectObject(dc, GetStockObject(NULL_BRUSH));\n");
        }

        int x1 = savedFigures[i][0] * sizeMaster;
        int y1 = savedFigures[i][1] * sizeMaster;
        int x2 = savedFigures[i][2] * sizeMaster;
        int y2 = savedFigures[i][3] * sizeMaster;
        ...
        if (savedFigures[i][5] == 0) {
            ...
            printf("\n    POINT polyCoords%s%d[200];\n", countOfCards, polyCounter);
            for (int p = 0; p < savedFigures[i][0]; p++){
                int x3 = polygonsCoords[polyCounter][p].x * sizeMaster;
                int y3 = polygonsCoords[polyCounter][p].y * sizeMaster;

                printf("    polyCoords%s%d[%d].x = %d;\n", countOfCards, polyCounter, p, x3);
                printf("    polyCoords%s%d[%d].y = %d;\n", countOfCards, polyCounter, p, y3);
                printf("\n");
            }
            int count = savedFigures[i][0];

            printf("    Polygon(dc, polyCoords%s%d, %d);\n", countOfCards, polyCounter, count);
            polyCounter += 1;
        }
        printf("\n");
    }
...
int main() {
    ...
    printf("Введите номер создаваемой локации (от 1 до 999): ");
    scanf("%s", countOfCards);

    for (size_t i = 0; i < 3-strlen(countOfCards); i++){
        strcat(copyCount, "0");
    }
    strcat(copyCount, countOfCards);
    strcpy(countOfCards, copyCount);
